 ...
 -> X  - '${str}'  : (${form})  `);
  ...
  function applyMasking(text, entities, tokenFor) {
    ...
    for (const entity2 of entities) {
      if (entity2.start < cursor) {
        continue;
      }
      output += text.slice(cursor, entity2.start);
      output += tokenFor(entity2);
      cursor = entity2.end;
    }
  ...
  function replaceKnownTokens(text, resolver) {
    ...
    return text.replace(TOKEN_REGEX, (fullMatch, innerToken) => {
  ...
  function resolveTabId(requestTabId, sender) {
  ...
  function notifyTabStateChanged(tabId, enabled, maskedCount) {
    const message = {
      kind: "stateChanged",
      enabled,
      maskedCount
  ...
  function handleSetEnabled(request) {
    return setTabEnabled(request.tabId, request.enabled);
  }
  chrome.runtime.onMessage.addListener(
    (message, sender, sendResponse) => {
      try {
        sendResponse(handleRequest(message, sender));
      } catch (error) {
